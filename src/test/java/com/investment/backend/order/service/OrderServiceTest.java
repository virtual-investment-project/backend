...
class OrderServiceTest {
        ...
        @Test
        @DisplayName("배틀 계좌 - 지정 종목 거래 성공")
        void createOrder_BattleAccount_TickerMatch_Success() {
                // given: battle ticker = "BTCUSDT", stockCode = "BTCUSDT" → 정상
                com.investment.backend.battle.entity.Battle battle = com.investment.backend.battle.entity.Battle
                                .builder()
                                .ticker("BTCUSDT")
                                .name("테스트 배틀")
                                .startAt(java.time.LocalDateTime.now().minusDays(1))
                                .endAt(java.time.LocalDateTime.now().plusDays(1))
                                .initialCapital(100000)
                                .memberCount(10)
                                .teamCount(2)
                                .build();
                ReflectionTestUtils.setField(testAccount, "battle", battle);

                CreateOrderRequest request = createBuyOrderRequest(); // stockCode = "BTCUSDT"
                when(accountRepository.findById(testAccountId)).thenReturn(Optional.of(testAccount));
                when(orderRepository.save(any(Order.class))).thenReturn(testBuyOrder);
        ...
        @Test
        @DisplayName("배틀 계좌 - 다른 종목 거래 실패")
        void createOrder_BattleAccount_TickerMismatch_Fail() {
                // given: battle ticker = "BTCUSDT", stockCode = "ETHUSDT" → 예외
                com.investment.backend.battle.entity.Battle battle = com.investment.backend.battle.entity.Battle
                                .builder()
                                .ticker("BTCUSDT")
                                .name("테스트 배틀")
                ...
                CreateOrderRequest request = createSellOrderRequest(); // stockCode = "ETHUSDT"
                when(accountRepository.findById(testAccountId)).thenReturn(Optional.of(testAccount));
        ...
        void fillBuyOrder_Success() {
                ...
                doNothing().when(stockHoldingsService).addHoldings(
                                any(Account.class), anyString(), anyString(), any(BigDecimal.class),
